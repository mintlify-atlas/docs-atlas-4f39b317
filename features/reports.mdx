---
title: "Generate and Export Yakult Sales Reports"
sidebarTitle: "Reports"
description: "Generate filterable sales reports with date ranges, product/client/vendor filters, and grouping by day, week, month, or year. Export as PDF or printable HTML."
---
...
<Warning>
  ...
  - `fechaInicio` or `fechaFin` is missing or not in `YYYY-MM-DD` format.
  - `fechaInicio` is after `fechaFin`.
  - `agrupacion` is not one of `dia`, `semana`, `mes`, or `anio`.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Header" icon="file-pdf">
    Report name, date range, applied filters, and the name and role of the user who generated it.
  </Card>
