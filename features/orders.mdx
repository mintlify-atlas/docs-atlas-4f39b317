---
title: "Create and Track Yakult Distribution Orders End to End"
sidebarTitle: "Orders"
description: "Orders track every Yakult sale from creation to delivery — with transactional stock deduction, driver assignment, status updates, and WhatsApp alerts."
---
...
<Tabs>
  ...
  <Tab title="Change Status">
    ...
    **Response:**
    ...
    The assigned `repartidor` receives a push notification when the status changes — unless the repartidor themselves initiated the change. The backend reads the `x-user-id` request header to identify the requestor; if the header value matches the order's `repartidorId`, no notification is sent.
  </Tab>
